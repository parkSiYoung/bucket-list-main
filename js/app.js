...
class BucketListApp {
     ...
     */
    openEditModal(id) {
        const item = BucketStorage.load().find(i => i.id === id);
        if (!item) return;
        this.editingId = id;
    ...
    createBucketItemHTML(item) {
        ...
        return `
            <div class="bucket-item ${completedItemClass} bg-white rounded-lg shadow-md p-4 flex items-center gap-3 hover:shadow-lg transition-shadow">
                <!-- 체크박스 -->
                <button
                ...
                <div class="flex gap-2 flex-shrink-0">
                    ...
                    <button
                        ...
                        onclick="app.handleDelete('${item.id}')"
                    >
                        삭제
                    </button>
                </div>
